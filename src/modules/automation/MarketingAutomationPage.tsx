...
const MarketingAutomationPage = () => {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Marketing Automation</h1>
        <p className="text-gray-500 dark:text-gray-400">Pre-built marketing workflows to engage customers</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {mockMarketingAutomations.map((automation) => (
          <Card key={automation.id} className="hover:shadow-lg transition-shadow">
            <CardHeader>
              <div className="flex items-start justify-between">
                ...
                <Button 
                  ...
                  variant="ghost" 
                  size="icon"
                  className={automation.status === "Active" ? "text-green-600" : "text-gray-600"}
                >
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-2">
                <Badge variant={automation.status === "Active" ? "green" : "gray"}>
                  {automation.status}
                </Badge>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-2xl font-bold">{automation.sent}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Conversions</p>
                  <p className="text-2xl font-bold text-green-600">{automation.recovered}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Revenue</p>
                  <p className="text-xl font-bold text-green-600">{formatCurrency(automation.revenue)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Success Rate</p>
                  <p className="text-xl font-bold">{automation.rate}%</p>
                </div>
              </div>
