// MultiSelect — searchable dropdown with checkboxes, select-all, clear, keyboard navigation
...
export default function MultiSelect({
  ...
  /* Display text (smart label logic) */
  const displayLabel = useMemo(() => {
    if (value.length === 0) return label ? `${label}: None` : "None";
    if (value.length === options.length) return label ? `${label}: All` : "All";
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuTrigger asChild>
        <button
          className={cn(
            "flex items-center justify-between border rounded-md bg-white dark:bg-zinc-900",
            "dark:border-zinc-700 text-sm px-3 h-9 w-full",
            "text-left",
            className
          )}
        >
      ...
      {/* Dropdown */}
      <DropdownMenuContent
        className="w-auto min-w-[12rem] max-w-[300px] p-0"
        align="start"
      >
        <div className="p-2 space-y-2">
          {/* Search */}
          <input
            className="w-full border rounded-md px-2 py-1 text-sm bg-gray-50 dark:bg-zinc-800 dark:border-zinc-700 
