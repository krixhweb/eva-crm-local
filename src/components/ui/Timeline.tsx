...
/* Timeline — shows ordered status updates (e.g., order tracking, delivery steps) */
const Timeline: React.FC<TimelineProps> = ({ items }) => {
  return (
    <div className="space-y-4">
      {items.map((item, index) => {
        const isFirst = index === 0;                // highlight first item
        const isLast = index === items.length - 1; // no connector line below last item
        const isCompleted = isFirst || item.status === "Delivered"; // logic for green check

        return (
          <div key={index} className="relative flex items-start gap-4 pl-8">
            
            {/* Left side: dot + vertical line */}
            <div className="absolute left-0 top-1 flex flex-col items-center">
              ...
              >
                {/* Green check for completed steps, small dot for pending */}
                {isCompleted ? (
